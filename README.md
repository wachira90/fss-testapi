...
Bottle app that echoes the HTTP method, headers, and any POSTed data (JSON/form/files). Great for debugging.
...
```bash
# Headers + GET query
curl -i "http://localhost:8080/debug?foo=bar&foo=baz"

# POST JSON
curl -i -X POST http://localhost:8080/debug \
...
# POST multipart with a file
curl -i -X POST http://localhost:8080/debug \
  -F "note=hi" -F "file=@/etc/hosts"
```

Want a Dockerfile or a minimal Kubernetes Service/Deployment yaml for this debug endpoint? I can add that too.
